/* ============================================================
   PIXO Therapy · UI compartida
   - Inyecta el sprite de iconos SVG
   - Renderiza la barra de navegacion (con estado activo)
   - Toggle de tema claro/oscuro (persistente en localStorage)
   Uso:  <script src="static/js/pixo-ui.js"></script>
         <div id="pixo-nav" data-active="patients"></div>
   ============================================================ */
(function () {
  // ---- sprite de iconos (Lucide-style, stroke currentColor) ----
  const SPRITE = `
...
<symbol id="i-gamepad" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="6" y1="12" x2="10" y2="12"/><line x1="8" y1="10" x2="8" y2="14"/><line x1="15" y1="13" x2="15.01" y2="13"/><line x1="18" y1="11" x2="18.01" y2="11"/><rect x="2" y="6" width="20" height="12" rx="6"/></symbol>
...
<symbol id="i-history" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></symbol>
...
<symbol id="i-refresh" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M3 21v-5h5"/></symbol>
...
<symbol id="i-camera" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.5 4h-5L7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3l-2.5-3z"/><circle cx="12" cy="13" r="3"/></symbol>
...
<symbol id="i-file" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></symbol>
...
<symbol id="i-logout" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></symbol>
  ...
  const NAV = [
    { key: 'dashboard', href: 'Dashboard B.html', icon: 'i-home', label: 'Dashboard' },
    { key: 'patients', href: 'patients.html', icon: 'i-users', label: 'Pacientes' },
    { key: 'games', href: 'games.html', icon: 'i-gamepad', label: 'Juegos' },
  ];

  function renderNav(host) {
    const active = host.getAttribute('data-active') || '';
    const links = NAV.map(n =>
      `<a class="item${n.key === active ? ' active' : ''}" href="${n.href}">` +
      `<svg class="ic"><use href="#${n.icon}"/></svg>${n.label}</a>`
    ).join('');
    host.className = 'ui menu pixo-nav';
    host.innerHTML =
      `<a class="item brand" href="Dashboard B.html"><svg class="ic"><use href="#i-hand"/></svg>PIXO Therapy</a>` +
      links +
      `<div class="right menu">` +
        `<a class="item theme-toggle" onclick="PIXO.toggleTheme()" title="Cambiar tema"><svg class="ic" id="theme-ic"><use href="#i-moon"/></svg></a>` +
        `<div class="item">Dra. Salas</div>` +
        `<div class="item" style="padding-right:6px;"><span class="avatar">DS</span></div>` +
        `<a class="item" href="login.html">Salir</a>` +
      `</div>`;
  }
  ...
  function applyTheme(theme) {
    ...
    if (ic) ic.firstElementChild.setAttribute('href', theme === 'dark' ? '#i-sun' : '#i-moon');
  }

  const PIXO = {
    toggleTheme() {
      const next = document.body.getAttribute('data-theme') === 'dark' ? 'light' : 'dark';
      try { localStorage.setItem('pixo-theme', next); } catch (e) {}
      applyTheme(next);
    },
  };
  window.PIXO = PIXO;
  ...
  function init() {
    ...
    // nav
    const host = document.getElementById('pixo-nav');
    if (host) renderNav(host);
    // tema persistido
    let saved = 'light';
    try { saved = localStorage.getItem('pixo-theme') || 'light'; } catch (e) {}
    applyTheme(saved);
  }
